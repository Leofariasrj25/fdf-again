...
/*   Updated: 2022/11/05 16:49:40 by lfarias-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_controls(t_app *data)
{
	...
	point.y = point.y + MENU_LINE_HEIGHT;
	print_str(data, &point, MENU_KEY_COLOR, "RIGHT CLICK : MOVE");
	point.y = point.y + MENU_LINE_HEIGHT;
...
void	write_colorschemes(t_app *data)
{
	...
	point.x = MENU_LINE_MARGIN;
	point.y = 550;
	print_str(data, &point, MENU_KEY_COLOR, " ==== COLORSCHEMES ==== ");
	point.y = point.y + MENU_LINE_HEIGHT;
	print_str(data, &point, MENU_KEY_COLOR, "1 : DEFAULT");
	point.y = point.y + MENU_LINE_HEIGHT;
	print_str(data, &point, MENU_KEY_COLOR, "2 : NEON PURPLE");
	point.y = point.y + MENU_LINE_HEIGHT;
	print_str(data, &point, MENU_KEY_COLOR, "3 : HACKER GREEN");
	point.y = point.y + MENU_LINE_HEIGHT;
}
